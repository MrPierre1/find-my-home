...
var data = {price: 700000, bedrooms: 4}

var dataProps = Object.keys(data)
// console.log(dataProps)

for (let index = 0; index < raleighJS.length; index++) {
    // const element = array[index];
    
   for (const key in data) {
       if (data.hasOwnProperty(key)) {
        //    const element = object[key];
        if ( raleighJS[index][key]) {
           var checkValue =  raleighJS[index][key] === data[key]
           if(checkValue){
            var found = raleighJS[index]
           console.log('dataKLEY', index)
           }
